...
textLabel.Size = UDim2.new(0, 600, 0, 100) -- Kích thước của TextLabel, điều chỉnh chiều rộng để phù hợp với thông báo
textLabel.Position = UDim2.new(0.5, -300, 0.3, -50) -- Vị trí, hiển thị phía trên giữa màn hình
textLabel.BackgroundTransparency = 1 -- Nền trong suốt
textLabel.TextColor3 = Color3.new(1, 0.84, 0) -- Màu chữ vàng giống trong game Blox Fruits
textLabel.Text = "Đây là bản script dành cho exploit pc free\ndo exploit pc mới vẫn chưa hỗ trợ nhiều \nnên script còn khá nhiều lỗi sẽ được update sau này"
textLabel.Font = Enum.Font.Arcade -- Phông chữ giống như trong game Blox Fruits
textLabel.TextScaled = true -- Tự động điều chỉnh kích thước chữ
textLabel.TextStrokeTransparency = 0 -- Viền chữ đen, làm nổi bật chữ
textLabel.TextStrokeColor3 = Color3.new(0, 0, 0) -- Màu viền chữ đen
textLabel.TextWrapped = true -- Cho phép chữ xuống dòng tự động nếu vượt quá chiều rộng

-- Hiệu ứng mờ dần khi thông báo xuất hiện và biến mất
...
if game.CoreGui:FindFirstChild("SpeedHubX") then
...
local mainopen = Instance.new("ImageButton")
local mainopens = Instance.new("UICorner")

mainopen.Name = "mainopen"
mainopen.Parent = SpeedHubX
mainopen.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
mainopen.Position = UDim2.new(0.101969875, 0, 0.110441767, 0)
mainopen.Size = UDim2.new(0, 45, 0, 45)
mainopen.Image = "rbxassetid://14035954412"
...
local Logo = Instance.new("ImageLabel")
	...
	Logo.Image = "http://www.roblox.com/asset/?id=14035954412"
...
ImageLabel.Image = "http://www.roblox.com/asset/?id=14035954412"
...
--------------------------------------------------------------------
local Library = Update:Window("       Hamster Hub","14356580214",Enum.KeyCode.RightControl);
------------------------------------------
                    ...
                    elseif game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                        ...
                        if game:GetService("Workspace").Enemies:FindFirstChild(NameMon) then
                            ...
                            if game:GetService("ReplicatedStorage"):FindFirstChild(NameMon) then
                                topos(game:GetService("ReplicatedStorage"):FindFirstChild(NameMon).HumanoidRootPart.CFrame * CFrame.new(0,20,0))
                            else    
                                topos(CFrameMon)
                            end
                        end
...
--------------------------
local infor = Library:Tab("Infor","rbxassetid://14035954412")
...
local Race = Library:Tab("Race V4","rbxassetid://14035954412")
...
local mod = Library:Tab("Mod - Troll","rbxassetid://14035954412")
...
mod:Seperator(" Mob - Troll ")
mod:Button("Remove Lave",function()
    ...
    mod:Button("Invisible (lag)",function()
    game.Players.LocalPlayer.Character.LowerTorso.Root:Destroy()
end)
    
    mod:Button("Invisible [Remove]",function()
...
hrp.CFrame = old
    ...
    mod:Button("Max Zoom", function()
        while wait() do
            game.Players.LocalPlayer.CameraMaxZoomDistance = 9223372036854718
            end
    end)

    mod:Button("Buddha Big", function()
...
mod:Seperator("\\\\\  Boost FPS   //")
    ...
    mod:Toggle("RTX Mode",_G.RTXMode,function(value)
        _G.RTXMode = value
    ...
    mod:Button("Remove Fog",function(value)
        game.Lighting.BaseAtmosphere:Destroy()
    end)
    
mod:Toggle("Super FPS boost",false,function(value)
...
if not game:GetService("UserInputService").TouchEnabled and not game:GetService("UserInputService").KeyboardEnabled == false then
    mod:Slider("FPS Lock",0,499,60,function(K)setfpscap(tonumber(K));
    end)
    else
mod:Slider("FPS Lock",0,499,60,function(K)setfpscap(tonumber(K));
    end)
    ...
    mod:Button("FPS Boost",function()
     ...
     mod:Button("Unlock Portal",function()
    ...
    mod:Button("Unlock FPS",function()
        setfpscap(500)
    end)

    mod:Button("FPS delete color",function()
...
v.Material = "Plastic"
    ...
    mod:Button("FPS Fast Mode",function()
for _,v in pairs(workspace:GetDescendants()) do
if v.ClassName == "Part"
or v.ClassName == "SpawnLocation"
or v.ClassName == "WedgePart"
...
mod:Seperator(" Highlight ")

mod:Toggle("Show Chat disabled", _G.chat, function(value)
    _G.chat = value
...
StarterGui:SetCoreGuiEnabled(Enum.CoreGuiType.Chat, true)    
  ...
  mod:Toggle("Show leaderboard disabled", _G.leaderboard, function(a)
    _G.leaderboard = a
...
mod:Toggle("Highlight Mode",false,function(value)
        if value == true then
...
mod:Toggle("Graphic",false,function(v)
if v then
    ...
    mod:Toggle("Xray",false,function(value)
        NoWorld = value
...
---- [RainbowHaki]
    ...
    mod:Toggle("Rainbow Haki",false,function(value)
        RainbowHaki = value
    end)

    mod:Toggle("Rainbow Yoru",false,function(value)
        RainbowYoru = value
    end)
...
local function gomQuaiVatTungTen(tenQuaiVat)
    ...
    local playerPosition = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") and localPlayer.Character.HumanoidRootPart.CFrame
    
    if enemies and playerPosition then
        for _, enemy in pairs(enemies:GetChildren()) do
            if enemy:IsA("Model") and enemy:FindFirstChild("HumanoidRootPart") and enemy.Name == tenQuaiVat then
                enemy.HumanoidRootPart.CFrame = playerPosition
                enemy.HumanoidRootPart.CanCollide = false
                enemy.HumanoidRootPart.Size = Vector3.new(50, 50, 50)
                if enemy:FindFirstChild("Humanoid") and enemy.Humanoid:FindFirstChild("Animator") then
...
spawn(function()
    while true do
        if _G.FastAttack then
            updateAllTools()
            attackContinuously(numberOfAttacks)
        end
    end
end)
local player = game.Players.LocalPlayer

-- Hàm để lấy tất cả công cụ mà người chơi đang có
...
-- Cập nhật thuộc tính công cụ để tắt hoàn toàn độ trễ (nếu có thể)
local function removeToolDelay(tool)
    if not tool then return end
    local handle = tool:FindFirstChild("Handle")
    if handle then
        local toolScript = handle:FindFirstChildOfClass("Script")
        if toolScript then
            local attackRate = toolScript:FindFirstChild("AttackRate")
            local cooldown = toolScript:FindFirstChild("Cooldown")
            if attackRate then
                attackRate.Value = 0.01 -- Giảm giá trị này xuống tối thiểu
            end
            if cooldown then
                cooldown.Value = 0.01 -- Giảm giá trị này xuống tối thiểu
            end
        end
    end
end

-- Thực hiện nhiều đòn đánh liên tục để tăng số lượng đòn đánh
local function attackContinuously(times)
    while true do
        local tools = getAllTools()
        for _, tool in pairs(tools) do
            if tool:IsA("Tool") and tool:FindFirstChild("Handle") then
                local handle = tool.Handle
                -- Tìm và gọi hàm tấn công của công cụ nếu có
                local attackFunction = handle:FindFirstChild("Attack")
                if attackFunction and attackFunction:IsA("BindableFunction") then
                    -- Tấn công nhiều lần liên tiếp
                    for i = 1, times do
        ...
        wait(0.01) -- Điều chỉnh thời gian chờ để kiểm soát tốc độ giữa các đợt tấn công
    end
end

-- Cập nhật thuộc tính của tất cả công cụ
local function updateAllTools()
    local tools = getAllTools()
    for _, tool in pairs(tools) do
        removeToolDelay(tool)
    end
end

-- Gọi hàm để cập nhật công cụ và bắt đầu tấn công liên tục với số lượng đòn đánh mong muốn
local numberOfAttacks = 1000
...
Main:Seperator(" Auto Boss Raid ")
Main:Toggle('Auto Rip Indra', false, function(ripindraf)
        RipIndra = ripindraf
...
Main:Button("Auto Farm Chest (Tween)",function()
...
Main:Button("Giant Fake Transform",function()
    ...
    local ArgsTransform = {
     ...
     Color1 = Color3.fromRGB(0, 255, 166),
     Color2 = Color3.fromRGB(0, 255, 166),
     Color3 = Color3.fromRGB(0, 255, 166),
    }
   
    Player.Character.Humanoid:LoadAnimation(ReplicatedStorage.Util.Anims.Storage["2"].RaceTransform):Play()
    game:GetService("Players").LocalPlayer.Character.RightHand.Size = Vector3.new(2, 2, 2)
    game:GetService("Players").LocalPlayer.Character.LeftLowerArm.Size = Vector3.new(2, 2, 2)
    game:GetService("Players").LocalPlayer.Character.RightLowerArm.Size = Vector3.new(2, 2, 2)
    game:GetService("Players").LocalPlayer.Character.LeftUpperArm.Size = Vector3.new(2, 2, 2)
game:GetService("Players").LocalPlayer.Character.RightUpperArm.Size = Vector3.new(2, 2, 2)
game:GetService("Players").LocalPlayer.Character.LowerTorso.Size = Vector3.new(2, 2, 2)
game:GetService("Players").LocalPlayer.Character.LeftLowerLeg.Size = Vector3.new(2, 2, 2)
game:GetService("Players").LocalPlayer.Character.LeftFoot.Size = Vector3.new(2, 2, 2)
game:GetService("Players").LocalPlayer.Character.RightFoot.Size = Vector3.new(2, 2, 2)
game:GetService("Players").LocalPlayer.Character.RightLowerLeg.Size = Vector3.new(2, 2, 2)
game:GetService("Players").LocalPlayer.Character.RightUpperLeg.Size = Vector3.new(2, 2, 2)
game:GetService("Players").LocalPlayer.Character.UpperTorso.Size = Vector3.new(2, 2, 2)
game:GetService("Players").LocalPlayer.Character.LeftHand.Size = Vector3.new(2, 2, 2)
    delay(1, function()
...
Main:Button("Mink Fake Transform",function()
 require(game:GetService("ReplicatedStorage").Notification).new("Mink V4 By TNG!"):Display();
 wait()
 setthreadcontext(5)
...
Main:Button("Fishman Fake Transform",function()
 require(game:GetService("ReplicatedStorage").Notification).new("Fishman V4 By TNG!"):Display();
 wait()
 setthreadcontext(5)
...
Main:Button("Sky Fake Transform",function()
 require(game:GetService("ReplicatedStorage").Notification).new("Sky V4 By TNG!"):Display();
 wait()
 setthreadcontext(5)
...
Main:Button("Ghoul Fake Transform",function()
 require(game:GetService("ReplicatedStorage").Notification).new("Ghoul V4 By TNG!"):Display();
 wait()
 setthreadcontext(5)
...
Main:Button("Cyborg Fake Transform",function()
 require(game:GetService("ReplicatedStorage").Notification).new("Cyborg V4 By TNG!"):Display();
 wait()
 setthreadcontext(5)
...
Main:Button("Human Fake Transform",function()
 require(game:GetService("ReplicatedStorage").Notification).new("Human V4 By TNG!"):Display();
 wait()
 setthreadcontext(5)
 ...
 spawn(function()
    ...
    pcall(function()
     ...
     for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
      ...
      StartMagnetBoneMon = false
     topos(CFrame.new(-10099.4668, 206.259537, 6244.40137))
     end
     end)
    end
    end
...
Weapon:Toggle("Auto Fram Bone",_G.Auto_Farm_Bone,function(value)
_G.Auto_Farm_Bone = value
_G.click = value
 StopTween(_G.Auto_Farm_Bone)
 end)
Weapon:Toggle("Auto Trade Bone",_G.Auto_Farm_Bone,function(autotradebonef)
...
local Mirragecheck = Race:Label("Mirage Island")
spawn(function()
        pcall(function()
            while wait() do
...
Race:Button("Remove Fog",function(value)
    game.Lighting.BaseAtmosphere:Destroy()
end)
...
Race:Toggle("Teleport To Gear[Beta]",false,function(value)
...
game.StarterGui:SetCore("SendNotification", {
        Title = "TNGHub"; -- the title (ofc)
        Text = "Mirage not Found!"; -- what the text says (ofc)
        Icon = "rbxassetid://13699229918"; -- the image if u want. 
...
local FM = Race:Label("Check Full Moon: Only Available At Sea 3")
task.spawn(function()
            while task.wait() do
                pcall(function()
                    if game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=9709149431" then
                        FM:Set("5/5 : Full Moon 100%")
                    elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=9709149052" then
                        FM:Set("4/5 : Full Moon 75%")
                    elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=9709143733" then
                        FM:Set("3/5 : Full Moon 50%")
                    elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=9709150401" then
                        FM:Set("2/5 : Full Moon 25%")
                    elseif game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=9709149680" then
                        FM:Set("1/5 : Full Moon 15%")
                    else
...
Race:Button("Teleport To Clock Room",function()
Game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(28286.35546875, 14895.3017578125, 102.62469482421875)
  topos(CFrame.new(29437.5801, 15067.0342, -87.9190063))
end)
Race:Button("Buy Gear",function(t)
  game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer('UpgradeRace','Buy')
  end)
Race:Toggle("Auto Buy Gear",false,function(t)
    _G.buygear = t
end)
spawn(function()
    while wait() do
...
Race:Toggle("Auto Fram Acient One",_G.Auto_Farm_Bone,function(value)
   _G.Auto_Farm_Bone = value
   _G.buytn = value
StopTween(_G.Auto_Farm_Bone)
end)
...
Game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(28286.35546875, 14895.3017578125, 102.62469482421875)
 ...
 Race:Seperator("Complete Trial")
    Race:Toggle("Auto Kill Player Trial",false,function(value)
        ...
        _G.killkid = value
            ...
            spawn(function()
                while wait() do
                if _G.killkid then
                    local center = Vector3.new(28728, 14891.4, -93.9866)
    
                    -- Define the maximum search distance within the area
                     local maxDistance = 250  -- Replace with your desired radius
                    ...
                    for _, player in ipairs(game.Players:GetPlayers()) do
                        ...
                        if character then
                            ...
                            local distance = getDistance(center, playerPosition)
                    ...
                    local targetPlayer = nearestPlayer.Name
                    local speed = 5 -- spin speed
                    local distance = 20 -- distance from the target
                    ...
                    local targetRoot = playerService[targetPlayer].Character.HumanoidRootPart
                    
                    
                    local spinTime = 0
                    local spinCn = runService.Heartbeat:Connect(function(deltaTime)
                       spinTime += deltaTime * speed
  ...
  Race:Button("Auto Complete Sky Trial",function()
...
Race:Button("Auto Complete Cyborg Trial",function()
  game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Workspace").Map.MinkTrial.Ceiling.CFrame * CFrame.new(0,-5,0)
  end)
Race:Toggle("Auto Complete Ghoul Trial",false,function(value)
_G.Kill_Aura = value
end)
spawn(function()
    while wait() do
...
Race:Toggle("Auto Complete Human Trial",false,function(value)
  _G.Kill_Aura = value
end)
spawn(function()
    while wait() do
...
Race:Toggle("Auto Complete Fish Trial [Beta]",_G.Skill,function(value)
_G.Skill = value
wait(60)
Game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(28286.35546875, 14895.3017578125, 102.62469482421875)
...
game:GetService("VirtualInputManager"):SendKeyEvent(false,Enum.KeyCode.X,false,game)
 ...
 Race:Seperator("Teleport To Pvp")
...
P:Toggle("Aim Bot Skill And Gun Fov Fully",false,function(a)
        AimBotFullFunction = a
    end)
    
    P:Toggle("Show Fov",false,function(a)
        ShowFov = a
    end)
    _G.FOVSize = 500
    P:Slider("Fov Size",1,700,_G.FOVSize,function(v)
        _G.FOVSize = v
    end)
_G.run = 4000
P:Slider("Health Run",1,12000,_G.run,function(value)
 _G.run = value
end)
P:Toggle("Auto Run When Low Health",false,function(value)
_G.kss = value
StopTween(_G.kss)
end)
spawn(function()
...
if game.PlaceId == 2753915549 then
while wait() do
if (game.Players.LocalPlayer.Character.Humanoid.Health) <= _G.run then
...
while wait() do
if (game.Players.LocalPlayer.Character.Humanoid.Health) <= _G.run then
    Game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(923.40197753906, 125.05712890625, 32885.875)
end
end
elseif game.PlaceId == 7449423635 then
while wait() do
...
Playerslistt = {}
    ...
    for i,v in pairs(game:GetService("Players"):GetChildren()) do
        table.insert(Playerslistt,v.Name)
    end
local chonplayer = P:Dropdown("Select Players",Playerslistt,function(value)
        _G.chonplayer = value
    end)
    ...
    P:Toggle("Spectate Player",false,function(value)
        ...
        local plr2 = game:GetService("Players"):FindFirstChild(_G.chonplayer)
        repeat wait(.1)
            game:GetService("Workspace").Camera.CameraSubject = game:GetService("Players"):FindFirstChild(_G.chonplayer).Character.Humanoid
        until chonplayers == false 
        game:GetService("Workspace").Camera.CameraSubject = game:GetService("Players").LocalPlayer.Character.Humanoid
    end)
    
    P:Toggle("Teleport Player",false,function(value)
        _G.Auto_Kill_Ply = value
        StopTween(_G.Auto_Kill_Ply)
    end)
    P:Slider("Height",0,100,0,function(v)
        _G.height = v
    end)
    spawn(function()
        while wait() do
            if _G.Auto_Kill_Ply then
                pcall(function()
                    if _G.chonplayer ~= nil then 
                        if game.Players:FindFirstChild(_G.chonplayer) then
                            if game.Players:FindFirstChild(_G.chonplayer).Character.Humanoid.Health > 0 then
                                repeat task.wait()
                                    AutoHaki()
                                    game.Players:FindFirstChild(_G.chonplayer).Character.HumanoidRootPart.CanCollide = false
                                    topos(game.Players:FindFirstChild(_G.chonplayer).Character.HumanoidRootPart.CFrame * CFrame.new(0,_G.height,0))
                                    spawn(function()
                                        pcall(function()
                                        end)
                                    end)
                                until game.Players:FindFirstChild(_G.chonplayer).Character.Humanoid.Health <= 0 or not game.Players:FindFirstChild(_G.chonplayer) or not _G.Auto_Kill_Ply
                            end
                        end
                    end
                end)
...
P:Toggle("Auto Fram Player",false,function(value)
        _G.Auto_Kill_Plyyy = value
        _G.Skill = value
        StopTween(_G.Auto_Kill_Plyyy)
    end)
    
    spawn(function()
        while wait() do
            if _G.Auto_Kill_Plyyy then
                pcall(function()
                    if _G.chonplayer ~= nil then 
                        if game.Players:FindFirstChild(_G.chonplayer) then
                            if game.Players:FindFirstChild(_G.chonplayer).Character.Humanoid.Health > 0 then
                                ...
                                until game.Players:FindFirstChild(_G.chonplayer).Character.Humanoid.Health <= 0 or not game.Players:FindFirstChild(_G.chonplayer) or not _G.Auto_Kill_Plyyy
                            end
                        end
                    end
                end)
    ...
    P:Toggle("Auto Fram Player [Mele]",false,function(value)
        _G.Auto_Kill_Ply = value
        StopTween(_G.Auto_Kill_Ply)
    end)
...
P:Toggle("Auto Fram Player [Gun]",_G.Auto_Kill_Player_Gun,function(value)
    ...
    while wait() do 
        pcall(function()
            if _G.Auto_Kill_Player_Gun then
                if game.Players:FindFirstChild(_G.chonplayer) then
                    for i,v in pairs(game:GetService("Workspace").Characters:GetChildren()) do
                        if v.Name == _G.chonplayer and v.Humanoid.Health > 0 then
                            repeat wait()
                                if (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude > 300 then
                                    topos(v.HumanoidRootPart.CFrame)
                                elseif (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).magnitude <= 300 then
                            ...
                            until game.Players:FindFirstChild(_G.chonplayer).Character.Humanoid.Health <= 0 or not _G.Auto_Kill_Player_Gun or not game.Players:FindFirstChild(_G.chonplayer)
                        end
                    end
                end
            else
...
P:Toggle("Auto Spam Trick",false,function(value)
_G.Skill = value
end)
...
P:Toggle("Lock Camera To Player",_G.AimbotEnabled,function(value)
        _G.AimbotEnabled = value
    end)
P:Button("TNG Hub Bay",function()
loadstring(game:HttpGet("https://raw.githubusercontent.com/nhutduy777/TNGAutoBouty/main/Fly.lua"))()
end)
P:Toggle("Auto Active Race V3",_G.AutoAgility,function(value)
...
P:Toggle("Auto Active Race V4",false,function(value)
        _G.AutoAgi = value
    end)
...
Teleport:Seperator(" WORLD ")
 ...
 Teleport:Button("Teleport to Seabeast",function()
  for i,v in pairs(game:GetService("Workspace").SeaBeasts:GetChildren()) do
  if v:FindFirstChild("HumanoidRootPart") then
  topos(v.HumanoidRootPart.CFrame*CFrame.new(70,400,150))
  end
  end
 end)
...
Dungeon:Toggle("Auto Fram Dungeon",false,function(value)
...
if World2 then
   ...
   DevilFruit:Toggle("ESP NPC",false,function(nec)
   NpcESP = nec
   end)
spawn(function()
    while wait() do
...
DevilFruit:Toggle("ESP Island",false,function(ies)
IslandESP = ies
end)
spawn(function()
    while wait() do
...
DevilFruit:Toggle("ESP Fruit",false,function(fec)
FruitESPe = fec
end)
spawn(function()
    while task.wait() do
   ...
   DevilFruit:Seperator(" Fake Mode ")
    
    DevilFruit:Textbox("Fake Level",true,function(value)
        game:GetService("Players").LocalPlayer.Data.Level.Value = value
    end)
    DevilFruit:Textbox("Fake Exp",true,function(value)
        game:GetService("Players").LocalPlayer.Data.Exp.Value = value
    end)
    DevilFruit:Textbox("Fake Beli",true,function(value)
        game:GetService("Players").LocalPlayer.Data.Beli.Value = value
    end)
    DevilFruit:Textbox('Fake Fragment', true, function(value)
        game:GetService("Players").LocalPlayer.Data.Fragments.Value = value
    end)
    DevilFruit:Textbox("Fake Points",true,function(value)
        game:GetService("Players").LocalPlayer.Data.Points.Value = value
    end)
    DevilFruit:Textbox("Fake Bounty",true,function(value)
...
local id = value
Misc:Textbox("Sever ID",true,function(value)
        id = value
    end)
Misc:Button("Join Sever ID",function()
 game:GetService("TeleportService"):TeleportToPlaceInstance(7449423635,""..id.."", game.Players.LocalPlayer)
end)
Misc:Button("Copy Sever ID",function()
 setclipboard(game.JobId)
game.StarterGui:SetCore("SendNotification", {
  Title = "Copy Id",
  Icon = "rbxassetid://13699229918",
  Text = "Copy Id Sever Success!"
})
end)
...
"SUB2UNCLEKIZARU"
    ...
    Misc:Dropdown("Selected Codes 2X Experience",{"Sub2Fer999","JCWK","StarcodeHEO","MagicBUS","KittGaming","Sub2CaptainMaui","Sub2OfficialNoobie","TheGreatAce","Sub2NoobMaster123","Sub2Daigrock","Axiore","StrawHatMaine","TantaiGaming","Bluxxy","SUB2GAMERROBOT_EXP1","Enyu_is_Pro"},function(value)
        _G.Code2X = value
    end)
    Misc:Button("Redeem Code 2X Experience",function()
        game:GetService("ReplicatedStorage").Remotes.Redeem:InvokeServer(_G.Code2X)
    end)
    Misc:Button("Redeem All Codes 2X Experience",function()
    ...
    Misc:Seperator(" Code Reset Stars")
    Misc:Dropdown("Selected Codes Reset Stars",{"SUB2GAMERROBOT_RESET1","SUB2UNCLEKIZARU"},function(value)
        _G.CodeSelect = value
    end)
    
    Misc:Button("Redeem Code Reset Stars",function()
        game:GetService("ReplicatedStorage").Remotes.Redeem:InvokeServer(_G.CodeSelect)
    end)
   Misc:Button("Redeem All Codes Reset Stars",function()
        function RedeemCode(value)
            game:GetService("ReplicatedStorage").Remotes.Redeem:InvokeServer(value)
        end
        for i,v in pairs(reset) do
            RedeemCode(v)
        end
    end)
...
Misc:Toggle("Dodge No Cooldown",false,function(value)
    ...
    Misc:Toggle("Have All Tribes V3",false,function(value)
        mcrp = value
    end)
local Agility = game:GetService("ReplicatedStorage").FX["Agility"]:Clone()
local Water = game:GetService("ReplicatedStorage").FX["Water Body"]:Clone()
local Heavenly = game:GetService("ReplicatedStorage").FX["Heavenly Blood"]:Clone()
local Energy = game:GetService("ReplicatedStorage").FX["Energy Core"]:Clone()
local Heightened = game:GetService("ReplicatedStorage").FX["Heightened Senses"]:Clone()
local Last = game:GetService("ReplicatedStorage").FX["Last Resort"]:Clone()
spawn(function()
        while wait() do
if mcrp then
Agility.Parent = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart
Water.Parent = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart
Heavenly.Parent = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart
Energy.Parent = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart
Heightened.Parent = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart
Last.Parent = game:GetService("Players").LocalPlayer.Character.HumanoidRootPart
end
end
end)
...
local Saber = Op:Label("Not Available: Saber")
local Rengoku = Op:Label("Not Available: Rengoku")
local Midnight_Blade = Op:Label("Not Available: Midnight Blade")
local Dragon_Trident = Op:Label("Not Available: Dragon Trident")
local Yama = Op:Label("Not Available: Yama")
local Buddy_Sword = Op:Label("Not Available: Buddy Sword")
local Canvander = Op:Label("Not Available: Canvander")
local Twin_Hooks = Op:Label("Not Available: Twin Hooks")
local Spikey_Trident = Op:Label("Not Available: Spikey Trident")
local Hallow_Scythe = Op:Label("Not Available: Hallow Scythe")
local Dark_Dagger = Op:Label("Not Available: Dark Dagger")
local Tushita = Op:Label("Not Available: Tushita")
        ...
        pcall(function()
            for i,v in pairs(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getInventoryWeapons")) do
                if v.Name == "Saber" then
                    Saber:Set("Available: Saber")
                end
                if v.Name == "Rengoku" then
                    Rengoku:Set("Available: Rengoku")
                end
                if v.Name == "Midnight Blade" then
                    Midnight_Blade:Set("Available: Midnight Blade")
                end
                if v.Name == "Dragon Trident" then
                    Dragon_Trident:Set("Available: Dragon Trident")
                end
                if v.Name == "Yama" then
                    Yama:Set("Available: Yama")
                end
                if v.Name == "Buddy Sword" then
                    Buddy_Sword:Set("Available: Buddy Sword")
                end
                if v.Name == "Canvander" then
                    Canvander:Set("Available: Canvander")
                end
                if v.Name == "Twin Hooks" then
                    Twin_Hooks:Set("Available: Twin Hooks")
                end
                if v.Name == "Spikey Trident" then
                    Spikey_Trident:Set("Available: Spikey Trident")
                end
                if v.Name == "Hallow Scythe" then
                    Hallow_Scythe:Set("Available: Hallow Scythe")
                end
                if v.Name == "Dark Dagger" then
                    Dark_Dagger:Set("Available: Dark Dagger")
                end
                if v.Name == "Tushita" then
                    Tushita:Set("Available: Tushita")
                 end
            end
        end)
    end
...
local Bartilo_Quest = Op:Label("Not Available: Bartilo Quest")
local Don_Swan_Quest = Op:Label("Not Available: Don Swan Quest")
local Kill_Don_Swan = Op:Label("Not Available: Kill Don Swan")

spawn(function()
    while task.wait() do
        if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 3 then
            Bartilo_Quest:Set("Available: Bartilo Quest")
        end

        if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("GetUnlockables").FlamingoAccess == nil then
            --Nothing
        else
            Don_Swan_Quest:Set("Available: Don Swan Quest")
        end

        if game:GetService("ReplicatedStorage").Remotes["CommF_"]:InvokeServer("ZQuestProgress", "Check") == 1 then
            Kill_Don_Swan:Set("Available: Kill Don Swan")
        end
    end
end)
...
local Shisui = Op:Label("Not Available: Shisui")
local Saddi = Op:Label("Not Available: Saddi")
local Wando = Op:Label("Not Available: Wando")
local True_Triple_Katana = Op:Label("Not Available: True Triple Katana")
        ...
        pcall(function()
            for i,v in pairs(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getInventoryWeapons")) do
                if v.Name == "Shisui" then
                    Shisui:Set("Available: Shisui")
                end
                if v.Name == "Saddi" then
                    Saddi:Set("Available: Saddi")
                end
                if v.Name == "Wando" then
                    Wando:Set("Available: Wando")
                end
                if v.Name == "True Triple Katana" then
                    True_Triple_Katana:Set("Available: True Triple Katana")
                end
            end
        end)
    end
...
local Superhuman = Op:Label("Not Available: Superhuman")
local Death_Step = Op:Label("Not Available: Death Step")
local Sharkman_Karate = Op:Label("Not Available: Sharkman Karate")
local Electric_Claw = Op:Label("Not Available: Electric Claw")
local Dragon_Talon = Op:Label("Not Available: Dragon Talon")
local God_Human = Op:Label("Not Available: God Human")

spawn(function()
    while task.wait() do
        if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuySuperhuman",true) == 1 then
            Superhuman:Set("Available: Superhuman")
        end
        if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyDeathStep",true) == 1 then
            Death_Step:Set("Available: Death Step")
        end
        if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuySharkmanKarate",true) == 1 then
            Sharkman_Karate:Set("Available: Sharkman Karate")
        end
        if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyElectricClaw",true) == 1 then
            Electric_Claw:Set("Available: Electric Claw")
        end
        if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyDragonTalon",true) == 1 then
            Dragon_Talon:Set("Available: Dragon Talon")
        end
    end
end)
...
local Kabu_cha = Op:Label("Not Available: Kabucha")
local Acidum_Rifle = Op:Label("Not Available: Acidum Rifle")
local Bizarre_Rifle = Op:Label("Not Available: Bizarre Rifle")
        ...
        pcall(function()
            for i,v in pairs(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getInventoryWeapons")) do
                if v.Name == "Kabucha" then
                    Kabu_cha:Set("Available: Kabucha")
                end
                if v.Name == "Acidum Rifle" then
                    Acidum_Rifle:Set("Available: Acidum Rifle")
                end
                if v.Name == "Bizarre Rifle" then
                    Bizarre_Rifle:Set("Available: Bizarre Rifle")
                end
            end
        end)
    end
...
local Dark_Coat = Op:Label("Not Available: Dark Coat")
local Ghoul_Mask = Op:Label("Not Available: Ghoul Mask")
local Swan_Glass = Op:Label("Not Available: Swan Glass")
local Pale_Scarf = Op:Label("Not Available: Pale Scarf")
local Valkyrie_Helm = Op:Label("Not Available: Valkyrie Helm")
...
spawn(function()
    while task.wait() do
        pcall(function()
            for i,v in pairs(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("getInventoryWeapons")) do
                if v.Name == "Saber" then
                    Dark_Coat:Set("Available: Dark Coat")
                end
                if v.Name == "Ghoul Mask" then
                    Ghoul_Mask:Set("Available: Ghoul Mask")
                end
                if v.Name == "Swan Glasses" then
                    Swan_Glass:Set("Available: Swan Glass")
                end
                if v.Name == "Pale Scarf" then
                    Pale_Scarf:Set("Available: Pale Scarf")
                end
                if v.Name == "Valkyrie Helmet" then
                    Valkyrie_Helm:Set("Available: Valkyrie Helmet")
                end
            end
        end)
    end
end)
local FM = infor:Label("Check Full Moon: Only Available At Sea 3")
local Mirragecheck = infor:Label("Mirage Island")
task.spawn(function()
            while task.wait() do
                pcall(function()
                    if game:GetService("Lighting").Sky.MoonTextureId=="http://www.roblox.com/asset/?id=9709149431" then
...
infor:Label("Script Make By TN Gaming")
infor:Label("YTB: TN Gaming")

infor:Button("Copy Youtube Link",function()
 setclipboard("https://www.youtube.com/@tngaminghack")
 end)
infor:Button("Copy Discord Link",function()
 setclipboard("https://discord.gg/W5fNe5xWSX")
 end)